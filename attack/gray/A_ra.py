...
class A_ra():
    ...
    def fakeUserInject(self, recommender):
        ...
        row, col, entries = [], [], []
        for u in self.fakeUser:
            sampleItem =  random.sample(list(set(list(range(self.itemNum)))),self.maliciousFeedbackNum)
            for i in sampleItem:
